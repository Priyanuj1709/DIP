...
double getRandomWieght2()  //-1.0 ~ 1.0
{
...
BackPropagation2::BackPropagation2(int input_num /*= 2*/, int hide1_num /*= 4*/, int output_num /*= 1*/)
	...
	output_num_(output_num)
{
	...
	for (int i = 0; i < output_num_; ++i)
		output_weight_[i] = new double[hide1_num_ + 1];
	output_error_ = new double[output_num_];
	output_ = new double[output_num_];
	initWeight();
}

BackPropagation2::~BackPropagation2()
{
	delete[] input_;
	for (int i = 0; i < hide1_num_; ++i)
...
const double* BackPropagation2::predict(double* input)
{
	forward(input);
	return output_;
}

void BackPropagation2::train(double** samples, double** results, int sample_num, double threshold)
{
...
void BackPropagation2::initWeight()
{
	for (int i = 0; i < hide1_num_; ++i) {
		for (int j = 0; j < (input_num_ + 1); ++j)
			hide1_weight_[i][j] = getRandomWieght2();
	}
	for (int i = 0; i < output_num_; ++i) {
		for (int j = 0; j < hide1_num_ + 1; ++j)
			output_weight_[i][j] = getRandomWieght2();
	}		
}

void BackPropagation2::forward(double* input)
{   
	//save input
	input_[0] = 1.0;
	...
	for (int i = 0; i < hide1_num_; ++i) {
		...
		hide1_output_[i + 1] = sigmoid2(hide1_output_[i + 1]);
	}
	//hide1 layer -> output layer
	for (int i = 0; i < output_num_; ++i) {
		output_[i] = 0.0;
		for (int j = 0; j < hide1_num_ + 1; ++j)
			output_[i] += hide1_output_[j] * output_weight_[i][j];
		output_[i] = sigmoid2(output_[i]);
	}
}

void BackPropagation2::backward(double* output)
{   
	// ÇóÎó²îÏî
	for (int i = 0; i < output_num_; ++i) {
	...
	for (int i = 0; i < output_num_; ++i) {
		for (int j = 0; j < hide1_num_ + 1; ++j)
			output_weight_[i][j] += rate_ * output_error_[i] * hide1_output_[j];
	}
}
